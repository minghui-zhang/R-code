...
cell_sf_tidy_DC <- merge(cell_sf_tidy_DC, DC_temporalAuto)
cell_DC_tempAuto <- cell_sf_tidy_DC %>%
                      filter(dwi.autocorr.p.value < 0.05/nrow(DC_cell))

# SC
SC_temporalAuto <- as.data.frame(SC_cell[, c("label", "p.value")]) %>%
                      mutate(dwi.autocorr.p.value = p.value)
cell_sf_tidy_SC <- cell_sf_tidy[cell_sf_tidy$intensity == "SC",] 
cell_sf_tidy_SC <- merge(cell_sf_tidy_SC, SC_temporalAuto)
cell_SC_tempAuto <- cell_sf_tidy_SC %>%
                      filter(dwi.autocorr.p.value < 0.05/nrow(SC_cell))
