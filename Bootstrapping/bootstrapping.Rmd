...
calc_median_date <- function(agri_pixels, land_cover, plant_or_harv) {
  if (plant_or_harv == 'plant') {
...
calc_land_cover_error <- function(nsims, agri_pixels, actual_case, classified_case, plant_or_harv) {
  SC_diffs <- numeric(nsims)
  DC_diffs <- numeric(nsims)
  soy_diffs <- numeric(nsims)
  
  for (sim in 1:nsims) {
    actual_medians <- calc_median_date(agri_pixels, actual_case, plant_or_harv)
    classified_medians <- calc_median_date(agri_pixels, actual_case, plant_or_harv)
...
harvest_lc_error <- calc_land_cover_error(100, agri_pixels, actual_case, classified_case, 'harvest')
plant_lc_error <- calc_land_cover_error(100, agri_pixels, actual_case, classified_case, 'plant')

hist(plant_lc_error$SC, main = 'SC plant lc error')
hist(plant_lc_error$DC, main = 'DC plant lc error')
hist(plant_lc_error$soy, main = 'soy plant lc error')

hist(harvest_lc_error$SC, main = 'SC harvest lc error')
hist(harvest_lc_error$DC, main = 'DC harvest lc error')
hist(harvest_lc_error$soy, main = 'soy harvest lc error')
