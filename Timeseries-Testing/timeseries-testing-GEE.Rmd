...
R_GEE_algorithm <- function(ts, window_EVI_1, window_EVI_2, window_dEVI, numMissingPts) {
  ...
  maxEVIday_fitted <- approx(x = fitted(model_GEE_harmonic), y = ts_fitting_R$t, xout = maxEVIfitted)$y
  
  results <- c(maxEVIday_fitted, quarterPd)
  names(results) <- c('maxEVIday_fitted', 'quarterPd')
  
  plot(ts_fitting_R$t, EVI_fitting, main = "GEE")
  ...
  abline(v = maxEVIday_fitted - quarterPd, col = "red")
  abline(v = maxEVIday_fitted - 2*quarterPd, col = "blue")
