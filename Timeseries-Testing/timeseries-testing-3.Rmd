...
ts <- read.csv('test_point3.csv')[,1:4]

colnames(ts) <- c('date', 'EVI_raw', 'EVI_smoothed', 'firstDeriv')
ts$date <- as.Date(ts$date, '%d-%h-%y')
ts$day <- as.numeric(ts$date - as.Date('2016-08-01')) # number of days
...
plot(t, EVI_raw)
points(t, EVI_smoothed1, col = 'red')
points(t, ts$EVI_smoothed, col = 'blue')

# calculate first derivative of smoothed EVI (forward 1st order difference) - test with test_point3.csv
...
calc_1st_deriv <- function(values) {
  ...
  for (index in 2:length(values)) {
    ...
    derivative <- values[index] - values[index - 1]
    derivatives[index] <- derivative
  }
  
  derivatives[0] <- derivatives[1] # to make sure the 1st derivative length still matches total time
...
plot(t, ts$firstDeriv, main = 'derivative, unsmoothed')
points(unique(t), dEVI, col = 'red')

# TEMPORARY: add calculated dEVI and EVI_smoothed2 to ts dataframe to compare
ts$R_EVI_smoothed2 <- EVI_smoothed2
ts$R_dEVI <- dEVI
